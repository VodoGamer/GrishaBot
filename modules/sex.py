        ...
        if not sex.get_send():
            if not sex.get_request():
                ...
                KEYBOARD.add(Text("Отказаться", payload={
                            "sex": "disagree"}))
                await message.answer(f"{await from_user.get_mention()} предложил посексить {await to_user.get_mention('datv')}",
                                    keyboard=KEYBOARD)
            else:
                await message.answer(f"У {await to_user.get_mention('gent')} есть незаконченный секс\nПопроси его поскорее потрахаться")
        else:
            await message.reply("У тебя есть незаконченный секс\nЧтобы закончить его напиши -секс")
    else:
        await message.answer(f"{await from_user.get_mention()} ёбнулся и начал ебать бота {await to_user.get_mention('gent')}👳‍♂️")
...
async def sex_disagree(message: Message):
    ...
    if await from_user.get_sex() == 1:
        await message.answer(f"{await from_user.get_mention()} отказалась от секса с {await partner_info.get_mention('ablt')}")
    else:
...
async def discard_sex_request(message: Message):
    ...
    try:
        ...
        await message.reply(f"{await from_user.get_mention()} завершил все запросы на секс")
    except:
