...
async def get_bonus(message: Message, user: User, chat: Chat):
    ...
    if last_bonus != []:
        cooldown = is_command_available(last_bonus[-1].date, timedelta(hours=6))
        if not cooldown[0]:
...
async def send_money(message: Message, match, user: User, chat: Chat):
    reply_user = await User.get(uid=message.reply_message.from_id, chat=chat)  # type: ignore
...
@bp.on.chat_message(regex=(r"(?i)^\.*\s*(список|лист|топ)\s*(форбс|forbes|богачей|денег)$"))
async def get_forbes_list(message: Message, chat: Chat):
    forbes_list = await User.filter(chat=chat).exclude(money=0).order_by("-money")
    top = await get_top_list(forbes_list, TopType.money)

    if top:
        await message.answer(f"Список Forbes беседы:\n {top}", disable_mentions=True)
    else:
