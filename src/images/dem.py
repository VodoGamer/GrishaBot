...
class Demotivator:
    ...
    def __init__(self, top_text='', bottom_text='', fill_color: str = "black",
                 top_size: int = 80, bottom_size: int = 60):
    ...
    def make(self, file: io.BytesIO) -> bytes:
        ...
        drawer.text(((1280 - size_1[2]) / 2, 840),
                    self.top_text, fill=self.font_color, font=font_1)

        if self.bottom_text:
            font_2 = self._test_font(self.bottom_size, width, self.bottom_text)
            size_2 = font_2.getbbox(self.bottom_text)
            drawer.text(
                ((1280 - size_2[2]) / 2, 930), self.bottom_text,
                fill=self.font_color, font=font_2)
